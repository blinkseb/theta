 ...
 * 
 * \c step_cov is only relevant if \c improve_cov is true. In this case, it controls is the step size factor to apply to the original step size for
 * evaluating the gradients for the covariance: For \c improve_* set to true, each parameter is varied from the minimum by \c step_cov times the initial step size. The

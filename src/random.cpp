...
const long int Random::ktab[128]= {
...
RandomSourceTaus::RandomSourceTaus() {
    set_seed(0);
}

void RandomSourceTaus::set_seed(unsigned int s) {
    if (s == 0)s = 1;
    s1 = (69069 * s) & 0xffffffffL;
    if (s1 < 2) s1 += 2;
    s2 = (69069 * s1) & 0xffffffffL;
    if (s2 < 8) s1 += 8;
    s3 = (69069 * s2) & 0xffffffffL;
    if (s2 < 16) s1 += 16;
...
RandomSourceMersenneTwister::RandomSourceMersenneTwister(): mti(N){
    mag01[0] = 0;
    mag01[1] = MATRIX_A;
    set_seed(5489U);
}

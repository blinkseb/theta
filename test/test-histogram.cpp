...
BOOST_AUTO_TEST_CASE(test_reset){
   ...
   Random rnd(rnd_src);
   const size_t nbins = 101;
   Histogram1D m(nbins, rnd.uniform(), rnd.uniform() + 2.0);
   for(size_t i=0; i<nbins; i++){
       m.set(i, 0.1*i);
   }
   Histogram1D m0=m;
   BOOST_CHECK(histos_equal(m0, m));
   
   //reset with the same binning:
   m.reset_n(nbins);
   BOOST_CHECK(nbins == m.size());
   m.reset_range(m.get_xmin(), m.get_xmax());
   BOOST_CHECK(m.get_xmin()==m0.get_xmin());
   BOOST_CHECK(m.get_xmax()==m0.get_xmax());
   BOOST_CHECK(m.get_nbins()==m0.get_nbins());
   BOOST_CHECK(m.get_sum()==0.0);
   for(size_t i=0; i<nbins; i++){
       BOOST_REQUIRE(m.get(i)==0.0);
   }
   
   // reset with different binning:
   m.set(0.0, 1.0);
   m.reset_range(m.get_xmin()-1, m.get_xmax()+1);
   BOOST_CHECK(m.get_sum()==1.0);
   m.reset_n(nbins*2);
   BOOST_CHECK(m.get_nbins()==2*nbins);
   BOOST_CHECK(m.get_xmin()==m0.get_xmin()-1);
   BOOST_CHECK(m.get_xmax()==m0.get_xmax()+1);
   BOOST_CHECK(m.get_sum()==0.0);
   ...
   //reset to no bins:
   m.reset_n(0);
   BOOST_CHECK(m.get_nbins()==0);
   BOOST_CHECK(m.getData()==0);
}

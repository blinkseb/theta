...
void tbb_model::get_prediction_impl(DataT<HT> & result, const ParValues & parameters) const{
    ...
    for(ObsIds::const_iterator o = observables.begin(); o!=observables.end(); ++o){
        result[*o];
    }
    parallel_for(blocked_range<size_t>(0, last_indices.size()), [&](const blocked_range<size_t> & r){
        ...
        for(size_t k=r.begin(); k!=r.end(); ++k) {
            ...
            hfs[i].apply_functor(copy_to<HT>(result[obs_li.first]), parameters);
            result[obs_li.first] *= coeffs[i](parameters);
            ++i;
            for(; i<obs_li.second; ++i){
                hfs[i].apply_functor(add_with_coeff_to<HT>(result[obs_li.first], coeffs[i](parameters)), parameters);
            }
        }
    });
}
